...
const products = [
  ...
  { brand: '极创工坊', model: 'JC15', energy: 321.9, power: 322.9, dischargeStable: 65, chargeStable: 65 }, { brand: '立花', model: '3号Ultra', energy: 331.0, power: 284.0, dischargeStable: null, chargeStable: null },
  { brand: '怡路', model: 'YL200', energy: 331.8, power: 298.0, dischargeStable: null, chargeStable: null },
];
...
function ProductTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload?: (typeof products)[number] & { name: string } }> }) {
  const product = payload?.[0]?.payload;
  if (!active || !product) return null;
  return <div className="min-w-48 rounded-xl border border-[#173f35]/15 bg-white p-3 text-xs shadow-xl"><div className="text-sm font-bold text-[#173f35]">{product.brand} · {product.model}</div><div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-[#65726d]"><span>能量密度</span><strong className="text-right font-mono text-[#17231f]">{product.energy} Wh/L</strong><span>功率密度</span><strong className="text-right font-mono text-[#17231f]">{product.power} W/L</strong><span>放电稳定</span><strong className="text-right font-mono text-[#17231f]">{product.dischargeStable ?? '—'} W</strong><span>充电稳定</span><strong className="text-right font-mono text-[#17231f]">{product.chargeStable ?? '—'} W</strong></div></div>;
}
  ...
  return (
    <main className="min-h-screen bg-[#f4f4ed] text-[#17231f]"><div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-8 lg:px-12 lg:py-8">
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
        <div className="rounded-[24px] border border-[#17231f]/10 bg-[#fffef9] p-5 sm:p-6"><div className="mb-2 flex items-center justify-between"><div><p className="text-xs font-bold tracking-[.14em] text-[#77837e] uppercase">密度地图</p><h2 className="mt-1 text-xl font-semibold">谁更接近右上角？</h2><p className="mt-1 text-xs text-[#77837e]">将鼠标移到圆点上，或在手机上轻触圆点查看产品</p></div><span className="rounded-full bg-[#e6f0eb] px-3 py-1 text-xs font-bold text-[#187c64]">越靠右上越强</span></div><ChartContainer config={chartConfig} className="mt-4 h-[360px] w-full aspect-auto"><ScatterChart margin={{ top: 15, right: 15, bottom: 25, left: 5 }}><CartesianGrid strokeDasharray="4 5" vertical={false} /><XAxis type="number" dataKey="energy" domain={[180, 380]} tickLine={false} axisLine={false}><Label value="能量密度 Wh/L" position="bottom" offset={4} /></XAxis><YAxis type="number" dataKey="power" domain={[100, 800]} tickLine={false} axisLine={false} width={42}><Label value="功率密度 W/L" angle={-90} position="insideLeft" offset={10} /></YAxis><ZAxis range={[110, 110]} /><ReferenceLine x={300} stroke="#9aa59f" strokeDasharray="3 4" /><ReferenceLine y={300} stroke="#9aa59f" strokeDasharray="3 4" /><ChartTooltip cursor={{ strokeDasharray: '3 3' }} content={<ProductTooltip />} /><Scatter data={ranked} name="product" className="cursor-crosshair"><Cell fill="#e7a334" />{ranked.slice(1).map((p) => <Cell key={p.name} fill="#187c64" fillOpacity={0.82} />)}</Scatter></ScatterChart></ChartContainer></div>

        <div className="overflow-hidden rounded-[24px] border border-[#17231f]/10 bg-[#fffef9]"><div className="flex items-center justify-between p-5 pb-3 sm:p-6 sm:pb-3"><div><p className="text-xs font-bold tracking-[.14em] text-[#77837e] uppercase">完整排名</p><h2 className="mt-1 text-xl font-semibold">15 款产品实时排序</h2></div><Button variant="ghost" size="sm" onClick={() => setWeight(50)} className="text-[#53615c]"><RotateCcw className="size-4" />重置</Button></div><div className="max-h-[395px] overflow-auto"><Table><TableHeader className="sticky top-0 z-10 bg-[#f2f2eb]"><TableRow><TableHead className="pl-6">排名</TableHead><TableHead>产品</TableHead><TableHead className="text-right">能量密度</TableHead><TableHead className="text-right">功率密度</TableHead><TableHead className="text-right">放电稳定</TableHead><TableHead className="text-right">充电稳定</TableHead><TableHead className="pr-6 text-right">综合分</TableHead></TableRow></TableHeader><TableBody>{ranked.map((p, i) => <TableRow key={p.name} className={i < 3 ? 'bg-[#fff8e9]' : ''}><TableCell className="pl-6 font-mono font-bold text-[#187c64]">{String(i + 1).padStart(2, '0')}</TableCell><TableCell><div className="font-medium">{p.brand}</div><div className="text-xs text-[#7a8580]">{p.model}</div></TableCell><TableCell className="text-right font-mono">{p.energy.toFixed(1)}</TableCell><TableCell className="text-right font-mono">{p.power.toFixed(1)}</TableCell><TableCell className="text-right font-mono">{p.dischargeStable == null ? '—' : `${p.dischargeStable} W`}</TableCell><TableCell className="text-right font-mono">{p.chargeStable == null ? '—' : `${p.chargeStable} W`}</TableCell><TableCell className="pr-6 text-right font-mono font-bold">{p.score.toFixed(1)}</TableCell></TableRow>)}</TableBody></Table></div></div>
      </section>
